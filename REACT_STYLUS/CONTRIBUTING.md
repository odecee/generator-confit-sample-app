...
Welcome! This document explains how you can contribute to making **react-stylus** even better.
...
react-stylus/
 ...
 │           ├──browserTest/      * system test specs for this module
 ...
 ├──prod/                         * production-build code is output here
 ...
 │
 ├──vrt/image/                    * visual regression testing reference images (if enabled)
 │
...
Command | Description
:------ | :----------
<pre>npm run build</pre> | Generate production build into [prod/](prod/) folder
...
<pre>npm run test:visual</pre> | Run visual regression tests against the *development* web server (the development server **must** be running)<ul><li>You can create visual regression tests by placing them in the `visualTest/` folder inside any of your module folders (e.g. `src/modules/demoModule/visualTest/`)</li><li>Example scenario:<pre>module.exports = {<br>  label: 'My Homepage',<br>  url: '/',<br>  selectors: ['header', 'footer']<br>};</pre></li></ul>
<pre>npm run test:visual:ref</pre> | Creates visual regression testing reference images which are subsequently used as the baseline for future tests. The development server **must** be running.
...
Command | Description
:------ | :----------
<pre>npm run pre-release</pre> | Verify code, run unit tests, check test coverage, build software.<ul><li>Define another script-task called `postrelease` to define the rest of your release process</li></ul>
...
- `npm run test:system` can be extended by modifying [config/testSystem/protractor.conf.js](config/testSystem/protractor.conf.js). This task uses Protractor to run Jasmine2 test specs (located in `src/modules/**/browserTest/`) against the development web server URL.
